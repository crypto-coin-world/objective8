...
(facts "about viewing comments for an objective"
       (binding [config/enable-csrf false]
         ...
         (fact "the remove comment button does not appear by default"
               (against-background
                 (http-api/get-objective OBJECTIVE_ID) => {:status ::http-api/success
                                                           :result {:uri  OBJECTIVE_URI
                                                                    :_id  OBJECTIVE_ID
               ...
               (-> user-session
                   (p/request (utils/path-for :fe/get-comments-for-objective
                                              :id OBJECTIVE_ID))
                   :response
                   :body) =not=> (contains "/meta/remove-comment"))

         (fact "remove comment button posts to the correct endpoint"
               (against-background
                 (http-api/get-objective OBJECTIVE_ID) => {:status ::http-api/success
                                                           :result {:uri  OBJECTIVE_URI
                                                                    :_id  OBJECTIVE_ID
               ...
               (-> user-session
                   ...
                   (p/request (utils/path-for :fe/get-comments-for-objective
                                              :id OBJECTIVE_ID))
                   :response
                   :body) => (contains "/meta/remove-comment"))
